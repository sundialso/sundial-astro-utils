...
{#
  ...
  Run selection: a run_group holding a terminal row wins over one that only ever
  wrote 'started'; ties break on latest, then run_group_id. A model whose only
  rows for a date are 'started' still reports 'started' — genuinely in flight.
...
{% macro create_dbt_completions_view() %}
  ...
  {# Per (model, execution_ts): pick the winning run_group. A run_group that
     reached a terminal ('succeeded'/'failed') outranks one that only ever wrote
     'started', however recent — an invocation that renders models but runs no
     hooks (a bare `dbt compile`; `execute` is true at compile time, so the
     start_ts()/end_ts() watermark MERGEs still insert a 'started' row) leaves a
     row nothing ever closes, and it must not mask a completed run. Within each
     of those tiers the latest run_group wins (max updated_at, then
     run_group_id for a deterministic tie-break), so a later re-run / backfill on
     the same date still supersedes an earlier one. #}
  ranked AS (
    SELECT
